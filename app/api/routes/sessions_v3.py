"""
API routes for live surgery sessions (v3 — OpenAI GPT-4.1-mini powered).

Drop-in replacement for sessions-v2 but using OpenAI vision API instead of Gemini.
Frontend contract is fully backward-compatible with sessions-v2.
Mount at /api/sessions-v3.
"""
...
from app.services.live_surgery_v3 import LiveSurgeryServiceV3
...
async def websocket_endpoint(
    ...
    """
    WebSocket endpoint for real-time surgical monitoring (v3 — OpenAI).
    Same frontend contract as sessions-v2.
    """
    await websocket.accept()
    logger.info("websocket_connected_v3", session_id=session_id)
    ...
    try:
        ...
        # Create v3 service (OpenAI-powered)
        service = LiveSurgeryServiceV3(db, session_id)
        ...
        while True:
            try:
                ...
                if message.get("type") == "websocket.disconnect":
                    logger.info("websocket_disconnect_received_v3", session_id=session_id)
                    break
                ...
                logger.info("websocket_disconnected_in_loop_v3", session_id=session_id)
                break
            except RuntimeError as e:
                if "disconnect" in str(e).lower():
                    logger.info("websocket_already_disconnected_v3", session_id=session_id)
                    break
                raise

    except WebSocketDisconnect:
        logger.info("websocket_disconnected_v3", session_id=session_id)
    except Exception as e:
        logger.error("websocket_error_v3", session_id=session_id, error=str(e))
    finally:
        if service:
